...
function AddExpensesModal({ show, onClose }) {
  ...
  const addCategoryHandler = async () => {
    ...
    if (!title || !color) {
      toast.error("Please provide both title and color");
      return;
    }
    ...
    <Modal show={show} onClose={onClose}>
      ...
      {expenseAmount > 0 && (
        <div className="flex flex-col gap-4 mt-6">
          ...
          {showAddExpense && (
            <div className="flex items-center justify-between">
              <label htmlFor="category-title" className="sr-only">Category Title</label>
              <input type="text" id="category-title" placeholder="Enter Title" ref={titleRef} />

              <label htmlFor="category-color">Pick Color</label>
              <input type="color" id="category-color" className="w-24 h-10" ref={colorRef} />
              <button
